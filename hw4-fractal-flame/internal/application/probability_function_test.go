...
func BenchmarkProbabilityIteratedFunction_RunIterations(b *testing.B) {
	...
	for _, threads := range threadCases {
		...
		b.Run(fmt.Sprintf("threads=%d", threads), func(b *testing.B) {
			p := NewProbabilityIteratedFunction(variations)

			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, _ = p.RunIterations(cfg)
			}
		})
...
func TestProbabilityIteratedFunction_RunIterations(t *testing.T) {
	...
	pixels, err := p.RunIterations(cfg)
